...
public class VodafoneCircularImageView extends ImageView {
    private static final String TAG = VodafoneCircularImageView.class.getSimpleName();
    ...
    private AllowanceValue.Type type;

    private int defaultBorderSize;
    private int defaultSelectorSize;

    public VodafoneCircularImageView(Context context) {
        this(context, null, R.styleable.CircularImageViewStyle_circularImageViewDefault);
    }

    public VodafoneCircularImageView(Context context, AttributeSet attrs) {
        this(context, attrs, R.styleable.CircularImageViewStyle_circularImageViewDefault);
    }

    public VodafoneCircularImageView(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init(context, attrs, defStyleAttr);
    }

    @TargetApi(Build.VERSION_CODES.LOLLIPOP)
    public VodafoneCircularImageView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        super(context, attrs, defStyleAttr, defStyleRes);
        init(context, attrs, defStyleAttr);
    }

    private void init(Context context, AttributeSet attrs, int defStyle) {
        defaultBorderSize = (int) (2 * context.getResources().getDisplayMetrics().density + 0.5f);
        defaultSelectorSize = (int) (2 * context.getResources().getDisplayMetrics().density + 0.5f);
        ...
        if (attrs != null) {
            ...
            hasBackgroundColor = attributes.getBoolean(R.styleable.CircularImageView_civ_has_backgroundColor, false);
            ...
            if (hasBackgroundColor) {
                backgroundColor = attributes.getColor(R.styleable.CircularImageView_civ_backgroundColor, getResources().getColor(R.color.transparent));
                setBackgroundFilter(backgroundColor);
            }

        } else { //// FIXME: 13/06/16 Dario!
            hasBorder = true;
            hasSelector = true;
        ...
        if (hasSelector) {
            ...
            setSelectorStrokeColor(Color.WHITE);
    ...
    /**
     * Sets the VodafoneCircularImageView's border width in pixels.
     *
    ...
    /**
     * Sets the VodafoneCircularImageView's basic border color.
     *
     ...
     * VodafoneCircularImageView. Be sure to provide some opacity.
     *
    ...
    /**
     * Sets the stroke width to be drawn around the VodafoneCircularImageView
    ...
    /**
     * Sets the stroke color to be drawn around the VodafoneCircularImageView
     * during click events when the selector is enabled.
     *
    ...
    public void setSelectorStrokeColor(int selectorStrokeColor) {
        if (paintSelectorBorder != null) {
            paintSelectorBorder.setColor(Color.parseColor("#70ffffff")); //// FIXME: 13/06/16 Dario o.O
        }
        this.invalidate();
    }

    public void setImageColorFilter(int imageColorFilter) {
        paint.setColorFilter(new PorterDuffColorFilter(imageColorFilter, PorterDuff.Mode.SRC_IN));
        this.invalidate();
    }

    public AllowanceValue.Type getType() {
        return type;
    }

    public void setType(AllowanceValue.Type type) {
        this.type = type;
    }
    ...
    public void onDraw(Canvas canvas) {
        ...
        if (hasSelector && isSelected) {
            ...
            paintSelectorBorder.setColorFilter(selectorFilter);
            canvas.drawCircle(center + outerWidth, center + outerWidth, ((canvasSize - (outerWidth * 2)) / 2) +
                    outerWidth - 4.0f, paintSelectorBorder);
